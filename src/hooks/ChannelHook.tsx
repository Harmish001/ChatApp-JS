...
const createChannel = async (body: any) => {
  const response = await axiosInstance.post("/add/channel", body);
  return response.data;
};

const addParticipant = async (body: any) => {
  const response = await axiosInstance.post("/add/participants", body);
  return response.data;
};
...
export const useCreateChannel = () => {
  return useMutation({
    mutationFn: (payload: any) => createChannel(payload),
  });
};

export const useAddParticipant = () => {
  return useMutation({
    mutationFn: (payload: any) => addParticipant(payload),
  });
};
